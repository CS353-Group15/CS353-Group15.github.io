...
$rs_enrolled = $mysqli->query("SELECT item_id, name FROM Contest WHERE item_id IN (SELECT contest_id FROM enroll WHERE username = '$username')");
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <div class="user-old-submission">
    ...
    <div class="user-all-announcements-content">
      ...
      <div class="user-all-announcements-content-bottom">
        <?php
        while ($row = mysqli_fetch_array($rs_enrolled)) :
          $id = $row['item_id'];
          $name = $row['name'];
          $enrolled = 1;
      ...
      <div class="user-all-announcements-content-footer">
        <a href="userAllContests.php" class="btn btn-primary">View all contests</a>
      </div>
    </div>
  </div>
